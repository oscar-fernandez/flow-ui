...
pipeline {
    ...
    stages  {
        ...
        stage("build-image") {
            ...
            steps {
                sh 'docker build . -t enablementprojects/flow-enablement-ui-dev:latest'
            }
        }
        stage("push-image") {
            when {
            ...
            steps {
                sh 'docker push enablementprojects/flow-enablement-ui-dev'
            }
        }
        stage("deploy-image") {
            when {
            ...
            steps {
                sh 'kubectl rollout restart deployment/flow-enablement-ui-dev --namespace=flow' 
            }
         }
    }
    post {
